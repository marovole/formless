...
-- 修复版：基础数据库架构 - 单独执行
-- =============================================
-- 说明：将 INSERT 语句分离，避免 ON CONFLICT 错误
-- =============================================
...
CREATE INDEX IF NOT EXISTS idx_key_quotes_user_id ON key_quotes(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_key_quotes_conversation_id ON key_quotes(conversation_id);
...
CREATE INDEX IF NOT EXISTS idx_api_keys_provider_priority ON api_keys(provider, priority ASC) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_api_keys_reset_at ON api_keys(reset_at);
...
CREATE INDEX IF NOT EXISTS idx_api_usage_api_key_id ON api_usage(api_key_id, created_at DESC);
...
-- 添加唯一约束
CREATE UNIQUE INDEX IF NOT EXISTS idx_prompts_name_unique ON prompts(name);
CREATE INDEX IF NOT EXISTS idx_prompts_role_language ON prompts(role, language) WHERE is_active = true;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_update_conversation_stats') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create own conversations') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can create messages in own conversations') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete messages in own conversations') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Service role can manage all quotes') THEN
...
-- =============================================
-- Initial Data (使用 WHERE NOT EXISTS 避免重复)
-- =============================================

-- Insert default prompts
INSERT INTO prompts (name, role, language, content, description)
SELECT * FROM (VALUES
...
) AS t(name, role, language, content, description)
WHERE NOT EXISTS (
  SELECT 1 FROM prompts WHERE prompts.name = t.name
);
