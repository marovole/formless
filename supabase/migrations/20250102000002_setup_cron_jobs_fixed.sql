...
-- 修复版：观照系统定时任务配置
-- =============================================
-- 修复说明：移除了不存在的 last_reset_at 字段引用
-- =============================================
--
...
BEGIN
  -- 如果任务已存在，先删除
  PERFORM cron.unschedule('guanzhao-check-rhythm-triggers');
EXCEPTION
...
DO $$
BEGIN
  PERFORM cron.unschedule('guanzhao-reset-daily-budget');
EXCEPTION
  WHEN undefined_table THEN NULL;
  WHEN OTHERS THEN NULL;
END $$;
...
DO $$
BEGIN
  PERFORM cron.unschedule('guanzhao-reset-weekly-budget');
EXCEPTION
  WHEN undefined_table THEN NULL;
  WHEN OTHERS THEN NULL;
END $$;
...
DO $$
BEGIN
  PERFORM cron.unschedule('guanzhao-cleanup-cooldowns');
EXCEPTION
  WHEN undefined_table THEN NULL;
  WHEN OTHERS THEN NULL;
END $$;
...
DO $$
BEGIN
  PERFORM cron.unschedule('guanzhao-cleanup-old-sessions');
EXCEPTION
  WHEN undefined_table THEN NULL;
  WHEN OTHERS THEN NULL;
END $$;
