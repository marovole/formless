-- =============================================
-- 观照（Guanzhao）主动 Agent 系统数据库架构
-- Migration: 20250102000001_guanzhao_system
-- =============================================
...
CREATE INDEX idx_user_sessions_last_activity ON user_sessions(user_id, last_activity_at DESC);
...
CREATE INDEX idx_guanzhao_history_user_id ON guanzhao_trigger_history(user_id, created_at DESC);
CREATE INDEX idx_guanzhao_history_trigger_id ON guanzhao_trigger_history(trigger_id);
CREATE INDEX idx_guanzhao_history_channel ON guanzhao_trigger_history(channel);
...
CREATE INDEX idx_guanzhao_budget_frequency ON guanzhao_budget_tracking(frequency_level);
...
CREATE INDEX idx_guanzhao_cooldowns_user_trigger ON guanzhao_cooldowns(user_id, trigger_id);
CREATE INDEX idx_guanzhao_cooldowns_until ON guanzhao_cooldowns(cooldown_until);
...
CREATE INDEX idx_safety_logs_user_id ON safety_screening_logs(user_id, created_at DESC);
CREATE INDEX idx_safety_logs_risk_level ON safety_screening_logs(risk_level);
CREATE INDEX idx_safety_logs_conversation ON safety_screening_logs(conversation_id);
...
DECLARE
  ...
  end_time TIME;
BEGIN
  SELECT dnd_start, dnd_end INTO dnd_start, dnd_end
  FROM guanzhao_budget_tracking
  WHERE user_id = is_user_in_dnd.user_id;
...
-- =============================================
-- pg_cron Jobs (需要启用 pg_cron 扩展后执行)
-- =============================================
